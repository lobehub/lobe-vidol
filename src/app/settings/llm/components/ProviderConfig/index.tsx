...
import { keyVaultsConfigSelectors, modelConfigSelectors } from '@/store/setting/selectors';
  ...
  ({
      ...
      !disableBrowserRequest &&
        (defaultShowBrowserRequest ||
          ...
          children: (
            <Switch
              onChange={(enabled) => {
                setConfig({ [LLMProviderConfigKey]: { [id]: { fetchOnClient: enabled } } });
              }}
    ...
    return (
      <Form
        ...
        form={form}
        items={[model]}
        onValuesChange={debounce(setConfig, 100)}
        {...FORM_STYLE}
      />
    );
  },
